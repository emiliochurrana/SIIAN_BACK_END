...
?>
		...
		<div class="w-100" id="cbody">
			...
			<div id='nw_msg'>Nova menssagem não lida</div>
		</div>
...
<div id="convo_clone" style="display: none">
	<div class="convo">
		...
		<small class="status">Enviando...</small>
	</div>
</div>
<style>
	#cheader{
...
<script>
	...
	$('#send_chat').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				...
				if(resp.status == 1){
					$('.convo .status[data-id="'+count+'"]').html('Enviada');
	...
	window.load_convo = function($convo_id='<?php echo $convo_id ?>'){
		$('#cfield').html("<center>Carregando conversa...</center>")
		$.ajax({
			...
			success:function(resp){
				if(resp){
					...
					if(Object.keys(resp).length > 0){
						...
						$('#cfield').html("<center>Sem dados</center>")
					}
				}
			},
			complete:function(){
